...
*)
   ...
   *)
    ...
    let ``08 An identifier pattern will match anything`` () =
        ...
        let y = "meetseashele" // replace with a string
        let z = 16.08 // replace with anything else!
    ...
    let ``09 A wildcard pattern will match anything`` () =
        ...
        let _ = "meetseashele" // replace with a string
        let _ = 16.08 // replace with anything else!
        ()

    [<Test>]
    let ``10 Constant patterns succeed if both sides match`` () =
        let 900 = 300 + 250 + 245 + 105
    ...
    let ``11 Constant patterns fail if the sides don't match exactly`` () =
        ...
        (fun () ->
            let "FILL ME IN" = "I don't want to fill you in!"
            ()
