...
module ``03: Putting the Function into Functional Programming`` = 
    ...
    let ``12 You can write a function as a one-liner (Part 1).`` () =
        (fun monday -> fun tuesday -> monday * tuesday) 9 3 |> should equal 27

    [<Test>]
    let ``13 You can write a function as a one-liner (Part 2).`` () =
        (fun monday tuesday -> monday + tuesday) 10 7 |> should equal 17
    ...
    let ``20 Functions have types`` () =
        ...
        let b r = 50.0 / r
        a "eat " ", goat!"|> should be ofType<string>
...
*)
    ...
    *)
    ...
    let ``34 Partially specifying arguments (Part 3).`` () =
        ...
        let f animal noise = animal + " says " + noise
        let cows i = f "cow" i + ", de gozaru" // <-- multiple words on this line, or you may want to make this a multi-line thing.  You MUST use `f`.
