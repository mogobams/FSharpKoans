    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
            let rec Length xs counter =
                match xs with
                | [] -> counter
                | head::tail -> Length tail (counter + 1) 
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
            let rec rev xs newlist =
                match xs with
                | [] -> newlist
                | head::tail -> rev tail ([ head ] @ newlist)
            rev xs [] // write a function to reverse a list here.
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let rec mapping xs newlist =
                match xs with
                | [] -> newlist
                | head::tail -> mapping tail (newlist @ [ head + 1 ])
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            let rec mapping xs newlist =
                match xs with
                | [] -> newlist
                | head::tail -> mapping tail (newlist @ [ head * 2 ])
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec mapping xs newlist =
                match xs with
                | [] -> newlist
                | head::tail -> mapping tail (newlist @ [ f head ])
            mapping xs [] // write a map which applies f to each element
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec filtering xs newlist =
                match xs with
                | [] -> newlist
                | head::tail -> match (f head) with
                                | true -> filtering tail (newlist @ [ head ])
                                | false -> filtering tail newlist
            filtering xs [] // write a function which filters based on the specified criteria
        ...
        let filter (xs : int list) : int list =
            let rec filtering xs newlist =
                ...
                | head::tail -> match (head % 2 = 0) with
                                | true -> filtering tail newlist
                                | false -> filtering tail (newlist @ [ head ])
            filtering xs [] // write a function to filter for odd elements only.
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            let rec sum xs state =
                match xs with
                | [] -> state
                | head::tail -> sum tail (state + head)
            sum xs initialState // write a function to do what's described above
    ...
    let ``15 A fold which multiplies a list`` () =
        let fold initialState xs =
            let rec product xs state =
                match xs with
                | [] -> state 
                | head::tail -> product tail (state * head)
            product xs initialState // write a function to multiply the elements of a list
    ...
    let ``16 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            let rec folding xs state =
                match xs with
                | [] -> state
                | head::tail -> folding tail (f state head)
            folding xs initialState // write a function to do a fold.
        ...
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
            let rec anymatch xs =
                ...
                | head::tail -> match (f head) with
                                | true -> true
                                | false -> anymatch tail 
            anymatch xs // Does this: https://msdn.microsoft.com/en-us/library/ee370309.aspx
    ...
    let ``19 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            let rec parting xs list1 list2 =
                match xs with
                | [] -> list1, list2
                | head::tail -> match (f head) with
                                | true -> parting tail (list1 @ [ head ]) (list2)
                                | false -> parting tail (list1) (list2 @ [ head ])
            parting xs [] [] // Does this: https://msdn.microsoft.com/en-us/library/ee353782.aspx
    ...
    let ``20 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec initiate size newlist =
                match size with
                | 0 -> newlist
                | _ -> initiate (size - 1) ([ f (size - 1) ] @ newlist)
            initiate n [] // Does this: https://msdn.microsoft.com/en-us/library/ee370497.aspx
    ...
    let ``21 tryFind: find the first matching element, if any`` () =
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
            let rec firstmatch xs =
                match xs with
                | [] -> None
                | head::tail -> match (p head) with
                                | true -> Some head
                                | false -> firstmatch tail 
            firstmatch xs // Does this: https://msdn.microsoft.com/en-us/library/ee353506.aspx
    ...
    let ``22 tryPick: find the first matching element, if any, and transform it`` () =
        let tryPick (p : 'a -> 'b option) (xs : 'a list) : 'b option =
            let rec firstmatch xs =
                ...
                | head::tail -> match (p head) with
                                | Some(_) -> p head
                                | None -> firstmatch tail
            firstmatch xs // Does this: https://msdn.microsoft.com/en-us/library/ee353814.aspx
    ...
    *)
    ...
    let ``23 choose: find all matching elements, and transform them`` () =
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
            let rec findall xs newlist =
                match xs with
                | [] -> newlist
                | head::tail -> match (p head) with
                                | Some(_) -> findall tail (newlist @ [ (p head).Value ])
                                | None -> findall tail newlist
            findall xs [] // Does this: https://msdn.microsoft.com/en-us/library/ee353456.aspx
    ...
    let ``24 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
            let rec mapindex xs newlist index =
                match xs with
                | [] -> newlist
                | head::tail -> mapindex tail (newlist @ [ f index head ]) (index + 1)
            mapindex xs [] 0 // Does this: https://msdn.microsoft.com/en-us/library/ee353425.aspx
