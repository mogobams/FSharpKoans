...
module ``07: The Good Kind of Discrimination`` = 
    ...
    let ``02 Creating & pattern-matching a discriminated union`` () = 
        ...
        randomOpinion (BSc (Mathematics, Statistics)) |> should equal "!!SCIENCE!!"
    ...
    let ``06 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        ...
        let a = 
            Node ("branch1", Node ("branch2", Empty, Node ("branch3", Node ("branch4", Empty, Empty), Empty)), Empty)// <-- you may want to spread this over multiple lines and/or let-bindings ...!
